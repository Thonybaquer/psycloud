import { db } from '@/db';
import { appointments, patients, userSettings } from '@/db/schema';
...
export default async function ControlPage() {
  ...
  const [newRegisteredRow] = (await (db as any).all(sql`
    ...
    WHERE p.created_at >= ${monthStart.toISOString()} AND p.created_at < ${nextMonthStart.toISOString()}
      AND COALESCE(p.created_source, 'new_patient') = 'new_patient'
      ...
      <div className="max-w-7xl mx-auto">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <ClockAlerts
              todayCount={(todayAppointments as any)?.length ?? 0}
