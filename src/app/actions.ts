...
import { db } from '@/db';
import { appointments, clinicalNotes, clinicalSessions, patients, userSettings } from '@/db/schema';
...
// -------------------------
// CALENDAR / TIME SETTINGS (stored in user_settings.settings_json.calendar)
// -------------------------
...
export type CalendarPrefs = {
  ...
  defaultDurationMinutes?: number; // 30/45/60
  enforceWorkHours?: boolean; // block creating outside hours
};

const DEFAULT_CAL_PREFS: Required<CalendarPrefs> = {
  workStart: '08:00:00',
  workEnd: '20:00:00',
  allowOverlap: false,
  use12h: true,
...
function sanitizeTime(t: unknown, fallback: string) {
...
function sanitizeInt(n: unknown, fallback: number, allowed: number[]) {
  const v = Number(n);
...
export async function getMyCalendarPrefs() {
  const { session } = await getAuth();
  const row = await db.select().from(userSettings).where(eq(userSettings.userId, session.uid)).get();
  let existing: any = {};
  try {
    existing = row?.settingsJson ? JSON.parse(row.settingsJson) : {};
  } catch {
    existing = {};
  }
  const cal = existing?.calendar ?? {};
  const prefs: Required<CalendarPrefs> = {
    workStart: sanitizeTime(cal.workStart, DEFAULT_CAL_PREFS.workStart),
    workEnd: sanitizeTime(cal.workEnd, DEFAULT_CAL_PREFS.workEnd),
    allowOverlap: !!cal.allowOverlap,
    use12h: cal.use12h === undefined ? DEFAULT_CAL_PREFS.use12h : !!cal.use12h,
    slotMinutes: sanitizeInt(cal.slotMinutes, DEFAULT_CAL_PREFS.slotMinutes, [15, 30, 60]),
    defaultDurationMinutes: sanitizeInt(cal.defaultDurationMinutes, DEFAULT_CAL_PREFS.defaultDurationMinutes, [30, 45, 60, 75, 90, 120]),
    enforceWorkHours: cal.enforceWorkHours === undefined ? DEFAULT_CAL_PREFS.enforceWorkHours : !!cal.enforceWorkHours,
  };
  return { success: true, prefs } as const;
}

export async function saveMyCalendarPrefs(input: CalendarPrefs) {
  const { session } = await getAuth();
  const now = new Date().toISOString();
  const row = await db.select().from(userSettings).where(eq(userSettings.userId, session.uid)).get();
  let existing: any = {};
  ...
  const merged = {
    ...
    calendar: {
      ...(existing.calendar ?? {}),
      workStart: sanitizeTime(input.workStart, (existing.calendar?.workStart ?? DEFAULT_CAL_PREFS.workStart)),
      workEnd: sanitizeTime(input.workEnd, (existing.calendar?.workEnd ?? DEFAULT_CAL_PREFS.workEnd)),
      allowOverlap: input.allowOverlap === undefined ? !!(existing.calendar?.allowOverlap) : !!input.allowOverlap,
      use12h: input.use12h === undefined ? (existing.calendar?.use12h ?? DEFAULT_CAL_PREFS.use12h) : !!input.use12h,
      slotMinutes: input.slotMinutes === undefined ? (existing.calendar?.slotMinutes ?? DEFAULT_CAL_PREFS.slotMinutes) : sanitizeInt(input.slotMinutes, DEFAULT_CAL_PREFS.slotMinutes, [15, 30, 60]),
      defaultDurationMinutes: input.defaultDurationMinutes === undefined
        ? (existing.calendar?.defaultDurationMinutes ?? DEFAULT_CAL_PREFS.defaultDurationMinutes)
        : sanitizeInt(input.defaultDurationMinutes, DEFAULT_CAL_PREFS.defaultDurationMinutes, [30, 45, 60, 75, 90, 120]),
      enforceWorkHours: input.enforceWorkHours === undefined
        ? (existing.calendar?.enforceWorkHours ?? DEFAULT_CAL_PREFS.enforceWorkHours)
        : !!input.enforceWorkHours,
    },
  };
  ...
  await (db as any).run(
    ...
    ON CONFLICT(user_id) DO UPDATE SET
      ...
      updated_at=excluded.updated_at
    `,
    [session.uid, JSON.stringify(merged), now]
  );
