...
import { db } from '@/db';
import { appointments, clinicalSessions, patients, userSettings } from '@/db/schema';
...
export default async function DashboardPage() {
  ...
  // User calendar/time preferences (for clock + calendar defaults)
  let use12h = true;
  try {
    const srow = await db.select().from(userSettings).where(eq(userSettings.userId, uid)).get();
    const json = srow?.settingsJson ? JSON.parse(srow.settingsJson) : {};
    if (json?.calendar?.use12h !== undefined) use12h = !!json.calendar.use12h;
  } catch {
    // ignore
  }
  ...
  const [totalApptsRow] = (await (db as any).all(sql`
    SELECT count(*) as count
    FROM appointments a
    WHERE (a.user_id = ${session.uid} OR (a.user_id IS NULL AND ${allowLegacy ? 1 : 0}))
  `)) as any[];
  ...
  const metrics = {
    ...
    // Si la base es nueva (sin citas creadas aquí), no mostramos este contador
    noFuture: (Number(totalApptsRow?.count ?? 0) || 0) > 0 ? (noFutureRow?.count ?? 0) : 0,
    noStructured: 0,
  };
