...
export function FinanceClient({
  ...
  yearPatientRows: Array<{ startIso: string; patientId: string; status: string }>;
}) {
...
{tab === 'graficos' && (
  <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 mt-6">
    ...
    <Card title="Pacientes y sesiones por mes" subtitle={`Volumen de atención en el año ${baseYear} (no todo es dinero).`}>
      <div className="h-[320px]">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={seriesPatientsMonth}>
            <CartesianGrid strokeDasharray="3 3" />
            <XAxis dataKey="label" />
            <YAxis />
            <Tooltip formatter={(v: any) => new Intl.NumberFormat('es-CO').format(Number(v))} />
      ...
      <div className="mt-2 text-xs text-slate-500 dark:text-slate-300">
        *Pacientes = conteo único por mes. Sesiones = número de citas (excluye canceladas).
      </div>
    </Card>


    <Card title="Gastos por categoría" subtitle={`Top categorías del año ${baseYear} (para saber en qué se va el dinero).`}>
      <div className="h-[320px]">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Tooltip formatter={(v: any) => new Intl.NumberFormat('es-CO').format(Number(v))} />
