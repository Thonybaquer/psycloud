...
export function ClockAlerts({ todayCount, appointments }: { todayCount: number; appointments: ClockAppt[] }) {
    ...
    const tick = () => {
      ...
      if (next) {
        ...
        if ([15, 10, 5].includes(minsToStart)) {
          ...
          toast.info(`En ${minsToStart} min inicia: ${next.patientName ?? 'Paciente'} (${fmtTime(next.start)}) · ${paid}`);
        }
      }
      ...
      if (current) {
        ...
        if (minsToEnd === 15) {
          ...
          if (!fireOncePerMinute(minuteKey)) return;
          toast.warning(`Quedan 15 min para terminar: ${current.patientName ?? 'Paciente'} (fin ${fmtTime(current.end)})`);
        }
      }
    };
