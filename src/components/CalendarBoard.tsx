...
type CalendarSettings = {
  ...
  defaultDurationMinutes: number; // 30/45/...
...
function loadSettings(): CalendarSettings {
  try {
    ...
    return {
      ...
      use12h: parsed.use12h === undefined ? DEFAULT_SETTINGS.use12h : !!parsed.use12h,
      slotMinutes: typeof parsed.slotMinutes === 'number' ? parsed.slotMinutes : DEFAULT_SETTINGS.slotMinutes,
      defaultDurationMinutes: typeof parsed.defaultDurationMinutes === 'number' ? parsed.defaultDurationMinutes : DEFAULT_SETTINGS.defaultDurationMinutes,
      enforceWorkHours: parsed.enforceWorkHours === undefined ? DEFAULT_SETTINGS.enforceWorkHours : !!parsed.enforceWorkHours,
    };
  } catch {
...
export function CalendarBoard() {
  ...
  useEffect(() => {
    ...
    // Then load from DB user_settings (authoritative)
    (async () => {
      try {
        const res = await getMyCalendarPrefs();
        if ((res as any)?.success && (res as any)?.prefs) {
          const prefs = (res as any).prefs as any;
  ...
  function isWithinWorkHours(start: Date, end: Date) {
    const [sh, sm] = settings.workStart.split(':').map((x) => Number(x));
    const [eh, em] = settings.workEnd.split(':').map((x) => Number(x));
    const dayStart = new Date(start);
    dayStart.setHours(sh || 0, sm || 0, 0, 0);
    const dayEnd = new Date(start);
    dayEnd.setHours(eh || 23, em || 59, 59, 999);
    return start >= dayStart && end <= dayEnd;
  }

  function roundToSlot(d: Date) {
    const slot = Math.max(5, Number(settings.slotMinutes) || 30);
    const ms = d.getTime();
    const slotMs = slot * 60_000;
    return new Date(Math.ceil(ms / slotMs) * slotMs);
  }

  function findNextFreeSlot(startFrom: Date, durationMinutes: number) {
    const dur = Math.max(5, Math.round(durationMinutes || 60));
    const slot = Math.max(5, Math.round(settings.slotMinutes || 30));
    ...
    const busy = (appts || [])
      ...
      .map((a) => {
        ...
        const e = a.endAt ? new Date(a.endAt) : new Date(s.getTime() + 60 * 60_000);
        return { s, e };
      })
      .filter((x) => !Number.isNaN(x.s.getTime()) && !Number.isNaN(x.e.getTime()));

    const start = roundToSlot(startFrom);
    for (let day = 0; day < horizonDays; day++) {
      const base = new Date(start);
      base.setDate(base.getDate() + day);

      // build day work window
      const [wh, wm] = settings.workStart.split(':').map((x) => Number(x));
      const [eh, em] = settings.workEnd.split(':').map((x) => Number(x));
      const dayStart = new Date(base);
      dayStart.setHours(wh || 0, wm || 0, 0, 0);
      const dayEnd = new Date(base);
      dayEnd.setHours(eh || 23, em || 59, 0, 0);

      // start searching from either start (day0) or dayStart
      let cursor = day === 0 ? new Date(Math.max(start.getTime(), dayStart.getTime())) : dayStart;
      cursor = roundToSlot(cursor);

      while (cursor.getTime() + dur * 60_000 <= dayEnd.getTime()) {
        const candidateEnd = new Date(cursor.getTime() + dur * 60_000);

        const overlaps = busy.some(({ s, e }) => cursor < e && candidateEnd > s);
        if (!overlaps) return { start: cursor, end: candidateEnd };

        cursor = new Date(cursor.getTime() + slot * 60_000);
      }
    }
    return null;
  }
  ...
  async function onSaveModal() {
    ...
    if (settings.enforceWorkHours && !isWithinWorkHours(s, e)) {
      toast.error(lang === 'en' ? 'Outside working hours' : 'Fuera del horario configurado');
      return;
    }
    ...
    <div className="pc-card bg-white dark:bg-slate-900">
      ...
      <div className="rounded-xl overflow-hidden bg-white dark:bg-slate-950 max-h-[550px] overflow-y-auto">
        <FullCalendar
          ...
          slotDuration={`00:${String(settings.slotMinutes).padStart(2, '0')}:00`}
          expandRows
          stickyHeaderDates
          scrollTime={settings.workStart}
          slotLabelFormat={{ hour: 'numeric', minute: '2-digit', hour12: settings.use12h, meridiem: settings.use12h ? 'short' : false }}
          eventTimeFormat={{ hour: '2-digit', minute: '2-digit', hour12: settings.use12h, meridiem: settings.use12h ? 'short' : false }}
          allDaySlot={false}
          nowIndicator
          editable
          selectable
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xl p-5">
            ...
            <div className="space-y-3">
              ...
              <div>
                <div className="text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">Intervalo del calendario</div>
                <select
              ...
              <div>
                <div className="text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">Duración por defecto</div>
                <select
                  value={settings.defaultDurationMinutes}
                  onChange={(e) => setSettings((s) => ({ ...s, defaultDurationMinutes: Number(e.target.value) }))}
                  className="w-full border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 bg-white dark:bg-slate-950"
                >
                  <option value={30}>30 min</option>
              ...
              <label className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-200">
                <input
                  ...
                  onChange={(e) => setSettings((s) => ({ ...s, use12h: e.target.checked }))}
                />
                Formato 12h (AM/PM)
              </label>
              ...
              <label className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-200">
                <input
                  type="checkbox"
                  checked={settings.enforceWorkHours}
                  onChange={(e) => setSettings((s) => ({ ...s, enforceWorkHours: e.target.checked }))}
                />
                Bloquear citas fuera del horario
              </label>
            </div>
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              <button
                type="button"
                onClick={async () => {
                  // Save locally for instant UX + persist in DB
                  saveSettings(settings);
                  try {
                    const res = await saveMyCalendarPrefs(settings as any);
              ...
              <div className="flex flex-wrap items-center gap-2">
                <button
                  ...
                  onClick={() => {
                    ...
                    setEndIso(new Date(s.getTime() + dur * 60_000).toISOString());
                  }}
                  className="px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-xs font-semibold"
                >
                ...
                <button
                  type="button"
                  onClick={async () => {
                    const startFrom = new Date(startIso || new Date().toISOString());
                    // Ensure we have appointments loaded a bit into the future
                    try {
                      const to = new Date();
                      to.setDate(to.getDate() + 45);
                      const rows = await listAppointmentsInRange(new Date().toISOString(), to.toISOString());
                      setAppts(rows as any);
                    } catch {
                      // ignore
                    }
                    const slot = findNextFreeSlot(startFrom, settings.defaultDurationMinutes || 60);
                    if (!slot) {
                      toast.error(lang === 'en' ? 'No free slot found' : 'No encontré un horario libre');
                      return;
                    }
                    setStartIso(slot.start.toISOString());
                    setEndIso(slot.end.toISOString());
                    toast.success(lang === 'en' ? 'Suggested time applied' : 'Horario sugerido aplicado');
                  }}
                  className="px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-xs font-semibold"
                >
