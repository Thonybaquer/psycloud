...
export function PatientSidebar({ patient, appointmentCount }: { patient: any; appointmentCount?: number }) {
  ...
  return (
    <aside className="w-80 h-screen bg-white border-r border-slate-100 p-6 flex flex-col gap-6 fixed left-0 top-0 overflow-y-auto z-10 hidden md:flex">
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-3 text-slate-600 bg-slate-50 p-3 rounded-xl">
          <div className="w-5 h-5 rounded-full bg-blue-100 flex items-center justify-center text-[11px] font-bold text-blue-700">
            #
          </div>
          <div>
            <p className="text-xs text-slate-400 uppercase">Citas</p>
            <p className="font-medium">{typeof appointmentCount === 'number' ? appointmentCount : '—'}</p>
          </div>
        </div>
      </div>
    </aside>
